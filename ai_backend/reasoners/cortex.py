...
async def execute_cortex(user_id: str, profile: SynapseSanitizedProfile) -> CortexMatchEvaluation:
    ...
    if vacs.data:
        ...
        vac = db.table("project_vacancies").insert({
            "project_id": proj_id,
    ...
    system_prompt = (
        "You are CORTEX, an AI matchmaking engine. Evaluate the candidate's sanitized profile against the required skills. "
        "Output a fair match_confidence_score between 0.0 and 1.0, and a concise 2-sentence justification explaining why."
    )
