...
async def execute_synapse(user_id: str, github_raw: dict, linkedin_raw: dict) -> SynapseSanitizedProfile:
    ...
    system_prompt = (
        "You are SYNAPSE, an expert technical recruiter AI. "
        "Analyze the provided raw GitHub metrics and LinkedIn history. "
        "Extract a unified list of sanitized_skills (e.g. ['Python', 'React', 'Docker']), "
        "write a compelling 2-sentence experience_summary summarizing their expertise, "
        "and map the raw metrics directly into the structured format required."
    )
    ...
    sanitized_profile = await agent.ai(
        ...
        user=user_prompt,
        schema=SynapseSanitizedProfile
    )
